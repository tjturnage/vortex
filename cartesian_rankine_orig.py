...
class VortexGrid:
        ...
        def __init__(self,dimension=200,rotmax_fraction=0.35,convergence = 0, translation = 0):
            ...
            # set up x and y grids and mesh them into a 2d grids
...
if rotv_surge:
    ...
    lc_surge = LineCollection(segments_surge, cmap=plts['brown_gray_ramp']['cmap'],norm=norm_rotv_surge,zorder=10)
...
if azshear_surge:
    ...
    segments_surge = np.concatenate([points_surge[:-1], points_surge[1:]], axis=1)
    lc_surge = LineCollection(segments_surge, cmap=plts['just_gray']['cmap'],norm=norm_rotv_surge,zorder=10)
    lc_surge.set_array(y)
